...
- Annual Recurring PR is implemented and development-verified: `/recurring-pr` owns annual schedule configuration and run history; schedules snapshot a source PR's eligible header/item fields and VAT rate, and the private Bangkok-date CLI creates only reviewable Drafts for active responsible users. Database uniqueness on `scheduleId + occurrenceYear` prevents duplicate annual runs. The worker serializably re-reads schedule state, skips newly future schedules before reference validation, then derives due occurrence before claiming a run and audits duplicate skips as System; it processes one annual occurrence per due schedule per invocation. Migration `000010_annual_recurring_pr`, concurrent worker/catch-up/Retry behavior, RBAC, and desktop/mobile browser flows were verified on 2026-07-15; the 2026-07-16 VAT/race hardening has repository coverage but no additional live smoke. See `docs/RECURRING_PR.md` for the workflow and operations contract.
