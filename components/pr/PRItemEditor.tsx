            ...
            {rows.map((row, index) => {
              ...
              return (
                <tr className={isHeadingRow ? "bg-blue-50/60" : isDetailRow ? "bg-slate-50/70" : undefined} key={row.id}>
                  ...
                  <td className={itemTableCompactCellClass}><select aria-label={`Row ${index + 1} type`} className={inputClass("px-2")} name={fieldNames.rowType} onChange={(event) => updateRow(row.id, "rowType", event.target.value)} value={row.rowType}><option value="ITEM">รายการ</option><option value="HEADING">หัวข้อ</option><option value="DETAIL">รายละเอียด</option></select></td>
                  <td className={itemTableCompactCellClass}><input aria-label={`Row ${index + 1} account code`} className={inputClass("px-2")} name={fieldNames.accountCode} onChange={(event) => updateRow(row.id, "accountCode", event.target.value)} placeholder={isPricedRow ? "Optional" : "-"} readOnly={!isPricedRow} value={isPricedRow ? row.accountCode : ""} /></td>
                  <td className={tableCellClass}><input aria-label={`Row ${index + 1} description`} className={inputClass(isHeadingRow ? "border-blue-200 bg-white font-bold text-blue-950" : isDetailRow ? "border-slate-200 bg-white pl-5 text-slate-700" : "")} name={fieldNames.description} onChange={(event) => updateRow(row.id, "description", event.target.value)} placeholder={isHeadingRow ? "หัวข้อกลุ่มรายการ" : isDetailRow ? "รายละเอียดต่อจากรายการ" : ""} required value={row.description} /></td>
                  <td className={itemTableCompactCellClass}><input aria-label={`Row ${index + 1} quantity`} className={numberInputClass} min="0.0001" name={fieldNames.quantity} onChange={(event) => updateRow(row.id, "quantity", event.target.value)} placeholder={isPricedRow ? "" : "-"} readOnly={!isPricedRow} required={isPricedRow} step="0.0001" type="number" value={isPricedRow ? row.quantity : ""} /></td>
                  <td className={itemTableCompactCellClass}><input aria-label={`Row ${index + 1} unit cost`} className={numberInputClass} min="0" name={fieldNames.unitCost} onChange={(event) => updateRow(row.id, "unitCost", event.target.value)} placeholder={isPricedRow ? "" : "-"} readOnly={!isPricedRow} required={isPricedRow} step="0.01" type="number" value={isPricedRow ? row.unitCost : ""} /></td>
