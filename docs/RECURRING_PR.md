...
`npm run recurring-pr:process` resolves today's Asia/Bangkok date and processes active schedules whose `nextRunDate` is on or before that date. A serializable worker transaction re-reads schedule status and checks whether its current `nextRunDate` is due before validating required references, then derives the annual occurrence before it claims an annual run, creates the Draft, reserves soft budget, links the run, and advances `nextRunDate`. A schedule paused before that transaction, or edited so its next run is no longer due, is skipped without a run, Draft, or audit; an inactive required reference on a still-due schedule produces one safe failed run, no Draft, and a System audit.
